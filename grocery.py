...
conn = Create_Db.create_connection("FoodGroups.db")   #sql.connect('FoodGroups.db')

FD_GROUP = Create_Db.create_table(conn=conn, create_table_sql='''CREATE TABLE IF NOT EXISTS FD_GROUP ([group_id]  text,[group_name] text)''')
F_DES = Create_Db.create_table(conn=conn, create_table_sql='''CREATE TABLE IF NOT EXISTS FD_DES(
